...
const DEMO = [
  ...
  { id: "d2", file: "auth/token.ts",       line: 22, type: "JWT Token",          severity: "medium", env: "JWT_SECRET",          fix: "Rotate JWT signing secret. Invalidate all active sessions.",                                         snippet: 'const token = "eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjoiYWRtaW4ifQ.s3cr3t";',   redacted: "eyJh●●●●c3t", rawLength: 64, detectionMethod: "pattern" },
  ...
  { id: "d4", file: "config/database.js", line:  8, type: "Database URL",        severity: "high",   env: "DATABASE_URL",        fix: "Rotate database credentials immediately.",                                                            snippet: "const db = new Pool({ connectionString: 'postgres://admin:p@ssw0rd@prod.db.io/app' });", redacted: "post●●●●app", rawLength: 55, detectionMethod: "pattern" },
  { id: "d5", file: "utils/email.js",     line:  5, type: "SendGrid Key",        severity: "high",   env: "SENDGRID_API_KEY",    fix: "Revoke at app.sendgrid.com/settings/api_keys.",                                                      snippet: 'sgMail.setApiKey("SG.xxxxxxxxxxxxxxxxxxx.yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy");', redacted: "SG.x●●●●yyy", rawLength: 69, detectionMethod: "pattern" },
...
export default function Ghostkey() {
  ...
  // AI call now hits our own server route — key stays server-side
  const fetchAI = useCallback(async finding => {
    ...
    try {
      ...
      setAiData(p => ({ ...p, [finding.id]: d.text || d.error || "Explanation unavailable." }));
    } catch {
      setAiData(p => ({ ...p, [finding.id]: "AI explanation unavailable — check your API key in Vercel env vars." }));
    } finally {
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: C.bg, color: C.text, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "52px 20px 60px" }}>
        ...
        <div className="fade-up" style={{ animationDelay: ".08s" }}
          ...
          style={{ border: `2px dashed ${dragOver ? C.accent : files.length ? C.accent + "55" : C.border}`, borderRadius: 14, padding: "30px 24px", textAlign: "center", cursor: files.length ? "default" : "pointer", background: dragOver ? `${C.accent}07` : files.length ? `${C.accent}04` : C.surface, transition: "all .2s ease", marginBottom: 14, boxShadow: dragOver ? `0 0 0 4px ${C.accent}18` : "none" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7, justifyContent: "center", marginBottom: 12 }}>
                {files.map(f => (
                  <div key={f.name} style={{ background: C.surf2, border: `1px solid ${C.border}`, borderRadius: 6, padding: "3px 10px", fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: C.muted, display: "flex", alignItems: "center", gap: 5 }}>
                    📄 {f.name}
                    <span onClick={e => { e.stopPropagation(); setFiles(files.filter(x => x.name !== f.name)); }} style={{ color: C.high, cursor: "pointer", fontSize: 15, lineHeight: 1, fontFamily: "sans-serif" }}>×</span>
                  </div>
                ))}
              </div>
        ...
        <div className="fade-up" style={{ display: "flex", gap: 10, marginBottom: 36, animationDelay: ".18s" }}>
          <button onClick={() => runScan(false)} disabled={!files.length && !pasted.trim()}
            style={{ flex: 1, padding: "12px 20px", borderRadius: 9, border: "none", background: (files.length || pasted.trim()) ? `linear-gradient(135deg,${C.accent},#008eb0)` : C.surf2, color: (files.length || pasted.trim()) ? "#000" : C.muted, fontSize: 14, fontWeight: 700, fontFamily: "'Syne', sans-serif", boxShadow: (files.length || pasted.trim()) ? `0 4px 22px ${C.accent}35` : "none" }}>
            🔍 Scan for Secrets
          </button>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <div style={{ width: "100%", maxWidth: 380, marginBottom: 22 }}>
          ...
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: C.accent, textAlign: "center" }}>
            {STEPS[Math.min(scanStep, STEPS.length - 1)]}<span className="cursor">▌</span>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "28px 20px 60px" }}>
        ...
        <div className="fade-up" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "22px 24px", marginBottom: 20, display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "center" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
              ...
              <span style={{ background: risk.color + "18", color: risk.color, border: `1px solid ${risk.color}35`, borderRadius: 6, padding: "2px 10px", fontSize: 10, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", letterSpacing: ".07em" }}>{risk.label}</span>
            </div>
                ...
                <div key={f.id} className="card fade-up" style={{ background: C.surface, border: `1px solid ${expanded === f.id ? sevColor(f.severity) + "45" : C.border}`, borderRadius: 13, overflow: "hidden", animationDelay: `${idx * .04}s` }}>
                  ...
                  {expanded === f.id && (
                    <div className="slide-down" style={{ borderTop: `1px solid ${C.border}`, padding: "18px 18px", display: "flex", flexDirection: "column", gap: 15 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 10, color: C.muted, fontFamily: "'JetBrains Mono', monospace", marginBottom: 6, letterSpacing: ".07em", textTransform: "uppercase" }}>🤖 AI Risk Analysis</div>
        ...
        {findings.length === 0 && (
          <div className="fade-up" style={{ textAlign: "center", padding: "52px 24px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14 }}>
            ...
            <p style={{ color: C.muted, fontSize: 14, lineHeight: 1.65, maxWidth: 380, margin: "0 auto" }}>No secrets detected. Keep it that way — always use environment variables and add <code style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>.env</code> to your <code style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>.gitignore</code>.</p>
          </div>
        )}
